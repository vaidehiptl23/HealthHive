...
const db = require('./config/database_simple');
...
app.get('/api/health/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    const records = await db.getHealthRecordsByUserId(userId);
    res.json({
      success: true,
      data: records
    });
...
app.get('/api/documents/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    const docs = await db.getDocumentsByUserId(userId);
    res.json({
      success: true,
      data: docs
    });
...
app.get('/api/family/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    const members = await db.getFamilyMembersByUserId(userId);
    res.json({
      success: true,
      data: members
    });
...
app.post('/api/family', authenticateToken, async (req, res) => {
  try {
    ...
    const member = await db.createFamilyMember({
      user_id: userId,
      name,
      relation,
      phone,
...
app.get('/', (req, res) => {
  res.json({ 
    message: 'HealthHive API Running (Simple In-Memory Version)',
    version: '1.0.0',
    status: 'active',
    note: 'Using in-memory database. Data will be lost on server restart.'
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 HealthHive Simple Backend running on port ${PORT}`);
  console.log(`📊 Environment: ${process.env.NODE_ENV}`);
  console.log(`🔗 API Base URL: http://localhost:${PORT}/api`);
  console.log(`💾 Database: In-memory (data persists until server restart)`);
  console.log(`👤 Test user: test@example.com / password123`);
  console.log(`🔑 Test user ID: 1`);
});